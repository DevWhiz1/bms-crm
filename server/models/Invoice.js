    ...
    try {
      ...
      const query = `
        SELECT 
          ...
          t.cnic as tenant_cnic,
          u.full_name as created_by_name,
          -- Calculate total amount using bill totals (not per-apartment)
          (
            CAST(mb.wapda_bill AS DECIMAL(10,2)) +
            CAST(mb.generator_bill AS DECIMAL(10,2)) +
            CAST(mb.water_bill AS DECIMAL(10,2)) +
      ...
      const billData = rows[0]
      
      // Get all apartments for this contract
      const apartmentsQuery = `
        SELECT a.apartment_no, a.floor_no
        ...
        ORDER BY a.floor_no, a.apartment_no
      `
      const apartments = await db.query(apartmentsQuery, [billData.contract_id])
      
      // Attach apartments list to bill data
      billData.apartments = apartments || []
