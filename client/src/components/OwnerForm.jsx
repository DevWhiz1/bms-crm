...
import { Close, PhotoCamera, Person, Badge, Phone } from '@mui/icons-material'
...
const OwnerForm = ({ open, onClose, owner, onSuccess }) => {
  ...
  useEffect(() => {
    if (owner) {
      ...
      setPhotoPreview(owner.photo_url || null)
      setCnicPhotoPreview(owner.cnic_photo_url || null)
    } else {
      reset({
        full_name: '',
        cnic: '',
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (cnicPhotoFile) formData.append('cnic_photo', cnicPhotoFile)

      if (isEdit) {
        await ownerAPI.updateOwner(owner.owner_id, formData)
      } else {
            ...
            <Grid item xs={12} md={4}>
              <Card sx={{ textAlign: 'center', p: 2 }}>
                ...
                <Box position="relative" display="inline-block">
                  ...
                  >
                    <PhotoCamera />
                    <input type="file" hidden accept="image/*" onChange={handleCnicPhotoChange} />
                  </IconButton>
                </Box>
                <Typography variant="caption" color="text.secondary">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    InputProps={{ startAdornment: <Badge sx={{ mr: 1, color: 'action.active' }} /> }}
                  />
                )}
              />
            </Grid>
